...
def income_function(y_test, y_pred, REVENUE = 500, BONUS = 100, PROBABILITTY = 0.5):
    ...
    # определяем сколько процентов от максимальной прибыли мы смоглибы получить
...
def gridsearch(function, param, cv_ = 5, metrik = income_function):
    ...
    # определяем параметрыы поиска оптимальной модели
...
def conclusion(optim_model, start, end):
    ...
    - модель с потимальными параметрами,
    ...
    # расcчитываем экономический эффект модели,
